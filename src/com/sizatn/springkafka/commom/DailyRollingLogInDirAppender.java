package com.sizatn.springkafka.commom;
...
/**
 * 继承DailyRollingFileAppender类写一个由时间和文件大小同时控制的类
 * 
 * @author
 *
 */
public final class DailyRollingLogInDirAppender extends
	 ...
	 *实现父类有参构造方法
	 */
	public DailyRollingLogInDirAppender(Layout layout, String filename,
	...
	public void doRollOver() {
		...
		if (maxBackupIndex > 0) {// 需要备件文件
			...
			// 移动文件名{(maxBackupIndex - 1), ..., 2, 1} to {maxBackupIndex, ...,
			// 3,2}
	...
	protected void subAppend(LoggingEvent event) {
		...
		checkFile();
		if ((fileName != null)
				&& ((CountingQuietWriter) qw).getCount() >= fileSize) {
			this.doRollOver();
		}
	}
